...
import {
  ...
  onSnapshot, doc, updateDoc, deleteDoc, Timestamp, getDocs
...
async function refreshPlanUI() {
  ...
  const usageCount = await getUsageCount(currentUser.uid);
  ...
  if (label) label.textContent = `${usageCount} / ${limit} appointments used`;
...
document.getElementById("bookBtn")?.addEventListener("click", async () => {
  ...
  // Check limit
  const { usageCount, limit } = await refreshPlanUI();
  if (usageCount >= limit) {
  ...
  try {
    // Estimate no-show risk via simple heuristic
    const hour = parseInt(time.split(":")[0]);
    const dayOfWeek = new Date(date).getDay();
    const isMonday = dayOfWeek === 1;
    const isLateAfternoon = hour >= 16;
    const riskLevel = (isMonday || isLateAfternoon) ? "high" : "low";
    ...
    if (document.getElementById("serviceType")) document.getElementById("serviceType").value = "";
    ...
    return `
      <div class="appt-card status-${appt.status || 'pending'} ${appt.noShowRisk === 'high' ? 'risk-high' : ''}">
        ...
        <div class="appt-info">
          ...
          ${appt.notes ? `<div class="appt-meta" style="margin-top:4px;font-style:italic;">💬 ${escapeHtml(appt.notes)}</div>` : ''}
