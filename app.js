...
function loadAppointments() {
  ...
  function attachSnapshot(uid) {
    ...
    onSnapshot(q, (snap) => {
      snap.docs.forEach(d => {
        if (!seenIds.has(d.id)) seenIds.add(d.id);
      });
      // Rebuild full list from both queries
      window.currentAppointments = [...seenIds].map(id => {
  ...
  attachSnapshot(currentUser);
  if (userEmail && userEmail !== currentUser) {
    attachSnapshot(userEmail);
  }
}
