...
// Auth: localStorage-based (username stored as "loggedInUser")
...
async function refreshPlanUI() {
  ...
  try {
    const planData = await getUserPlan(currentUser);
    currentPlan = planData?.plan || "free";
  } catch (e) { currentPlan = "free"; }

  try {
    usageCount = await getUsageCount(currentUser);
  } catch (e) { usageCount = 0; }
  ...
  if (chip) chip.style.display = pct >= 80 ? "block" : "none";
...
document.getElementById("bookBtn")?.addEventListener("click", async () => {
  ...
  const service    = document.getElementById("serviceType")?.value || "General";
  ...
  try {
    ...
    const noShowRisk = (dayOfWeek === 1 || hour >= 16) ? "high" : "low";

    console.log("📝 Attempting to book appointment for user:", currentUser);
    console.log("📦 Data:", { clientName, phone, date, time, service, duration });
    ...
    try { await incrementUsage(currentUser); } catch (e) {}
...
function loadAppointments() {
  ...
  const apptMap = new Map(); // keyed by doc id — prevents duplicates on re-fires

  function onApptsSnapshot(snap) {
    snap.docs.forEach(d => apptMap.set(d.id, { id: d.id, ...d.data() }));
    window.currentAppointments = [...apptMap.values()].sort(
      (a, b) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)
    );
    renderAppointments();
  }

  // Query by username (primary)
  onSnapshot(
    query(collection(db, "appointments"), where("uid", "==", currentUser)),
    onApptsSnapshot,
    (err) => console.error("App snapshot error:", err)
  );

  // Query by email (Google auth users may have uid = email)
  if (userEmail && userEmail !== currentUser) {
    onSnapshot(
      query(collection(db, "appointments"), where("uid", "==", userEmail)),
      onApptsSnapshot,
      (err) => console.error("App email snapshot error:", err)
    );
  }
}
...
function renderAppointments() {
  ...
  container.innerHTML = list.map(appt => {
    ...
    const durTag     = appt.duration ? `<span class="tag tag-service">${appt.duration}min</span>` : "";
    ...
    return `
      <div class="appt-card status-${appt.status || 'pending'} ${appt.noShowRisk === 'high' ? 'risk-high' : ''}">
        ...
        <div class="appt-info">
          ...
          <div class="appt-tags">${statusTag}${serviceTag}${durTag}${riskTag}</div>
        </div>
        <div class="appt-actions">
          ...
          <button class="appt-action-btn" onclick='generateReminder(${JSON.stringify(appt)})'>📱 Remind</button>
